...
def env_working(a,b,c,d,e):
	...
	if not os.path.exists(a + '.out'):
		cmd = GEAR + '/enva_rec3 -c ' + a + '.pdb > ' + a + '.out'
		subprocess.call(cmd,shell=True)
	if not os.path.exists(a + '_bb.out'):
		cmd1 = GEAR + '/enva_rec3 -b ' + a + '.pdb > ' + a + '_bb.out'
		subprocess.call(cmd1,shell=True)
	if not os.path.exists(a + '_aa.out'):
		cmd3 = GEAR + '/enva_rec3 -a ' + a + '.pdb > ' + a + '_aa.out'
		subprocess.call(cmd3,shell=True)
	if not os.path.exists('feat_rmsd.txt'):
		cmd2 = GEAR + '/rmsd_het ligand_feat.txt ' + b  + ' ' + a + ' >> feat_rmsd.txt'
		...
		with open(a + '_aa.out','r') as af :
			...
			for line in lines:
				if line.startswith('ATOM') >0 :
					...
					if c[i].split('_')[0] == line[22:26].strip() and c[i].split('_')[1] == line[17:20].strip() and c[i].split('_')[2] == line[12:16].strip() :
	...
	del_racc = 0
	del_racc = tot_racc - e #tot_mt_racc
	tidx = ' '
	tot_lig_acc = 0
	ave_lig_acc = 0
	tot_lig_num = 0
	...
	if tot_lig_num > 0 :
		...
		if not os.path.exists(a + '.ser'):
			with open(a + '.ser','w') as ff4:
				for sid in sidx:
					ff4.write('%s\n'%(sid))
	cmd3 = GEAR + '/iskew ' + a + '.ser >> ../' + d + '_energy_matrix/total_sk.txt'
	subprocess.call(cmd3,shell=True)
		
	with open('../' + d + '_energy_matrix/total_sk1.txt','w') as ff5:
		if idx1 == 0 :
			ff5.write('PDB\tskewness\tClass\tDecision\n')
		with open('../' + d + '_energy_matrix/total_sk.txt','r') as ff6:
			lines = ff6.readlines()
			for line in lines:
				if line.split('\t')[1] == 'nan':
	...
	with open('../' + d + '_energy_matrix/total_rac_ct.txt','a') as rf :
		if idx1 == 0 :
			rf.write('PDB')
			if len(rfeats_atm) > 0 :
	...
	n_bb = 0 
	with open('../' + d + '_energy_matrix/total_hh_ct.txt','a') as hf :
		if idx1 == 0 :
		...
		with open(a + '_bb.out','r') as hh :
			...
			for hline in hlines:
				for i in range(len(c)):
					if c[i].split('_')[0] == hline[22:26].strip() and c[i].split('_')[1] == hline[17:20].strip() and c[i].split('_')[2] == hline[12:16].strip() :
						n_bb = n_bb + 1
		hf.write('%s\t%d\t%d\n'%(a,len(hlines),n_bb))
	idx1 = idx1 + 1
...
def acc_chem_count(sam,lfeats,rfeats,ref):
	...
	fff = []
	for i in pdbs:
		fff.append(i.split('.')[0])
	pool.multiprocessing.Pool(24)
	prod_env=partial(env_working,b=ref,c=rfeats,d=sam,e=tot_mt_racc)
	atoms = []
	pool.map(prod_env,fff)
	pool.close()
	pool.join()
...
if len(sys.argv) == 1 :
	...
	if len(sam.split('_')) > 2 :
		lig = sam.split('-')[0]#sam.split('_')[0] #sam.split('_')[len(sam.split('_'))-1]
		rec = sam.split('-')[1]#sam.split('-')[0] #sam.split('_')[0] + '_' + sam.split('_')[1]
	else :
		lig = sam.split('-')[0]#sam.split('_')[0] #sam.split('_')[1]
		rec = sam.split('-')[1]#sam.split('-')[0] #sam.split('_')[0]
