...
def enva_working(pdb):
	inpdb = pdb.split('.')[0]
	csplit_cmd = 'csplit -f "%s_" %s.pdb \'/TER/\' --quiet ;sed \'s/ATOM  /HETATM/\' %s_01 > %s_02; cat %s_00 %s_02 > %s_het.pdb'%(inpdb,inpdb,inpdb,inpdb,inpdb,inpdb,inpdb)
	subprocess.call(csplit_cmd,shell=True)
	cp_cmd = 'cp %s.pdb %s.pdb'%(inpdb,inpdb.split('_')[-3])
	subprocess.call(cp_cmd,shell=True)
	if not os.path.exists(inpdb + '_a.out'):
		os.system(GEAR + '/enva.v2 -a ' + inpdb + '.pdb > ' + inpdb + '_a.out')
	if not os.path.exists(inpdb + '_b.out'):
		os.system(GEAR + '/enva.v2 -b ' + inpdb + '_het.pdb > ' + inpdb + '_b.out')
	if not os.path.exists(inpdb + '_m.out'):
		os.system(GEAR + '/enva.v2 -m ' + inpdb.split('_')[-3] + '.pdb ' + inpdb.split('_')[-3] + 'B > ' + inpdb + '_m.out')
	if not os.path.exists('*.env'):
		os.system(GEAR + '/enva.v2 -e ' + inpdb + '.pdb B')
...
def txt_writing(x):
	...
	for f,g in zip(sorted(glob.glob('*a.out')),sorted(glob.glob('*m.out'))):
		sidx= []
		pdb = '_'.join([f.split('.')[0].split('_')[0],str(x),f.split('.')[0].split('_')[2]])
		pdbl.append(pdb)
	...
	for f in sorted(glob.glob('*b.out')):
		...
		ff_list.append(f.split('.')[0].split('_')[0]+ '_' + str(x) + '_' + f.split('.')[0].split('_')[-4])
	...
	for f in sorted(glob.glob('*.env')):
		pdb = '_'.join([sam.split('_')[1],str(x),f.split('.')[0].split('_')[1]])
		pdbl.append(pdb)
		with open(f,'r') as F:
			acc = []
			psi = []
...
def reT(x):
	...
	with open(x.split('.')[0] + '_red.pdb','w') as W:
		for line in recl:
			W.write(line + '\n')
		W.write('TER\n')
		for line in pepl:
...
def work_part(x):
	...
	pool1.map(reT,sorted(glob.glob('*[0-9].pdb')))
	pool1.close()
	pool1.join()
	...
	multi_part1(lst1)
	os.system('rm -rf *_02 *_01 *_00 *_red.pdb')
	...
	pool2.map(reT,sorted(glob.glob('*[0-9].pdb')))
	pool2.close()
	pool2.join()
	for i in sorted(glob.glob('*[0-9].pdb')):
	...
	multi_part1(lst2)
	os.system('rm -rf *pdb_0* *_red.pdb')
	#subprocess.call(cmd,shell=True)
	print "########## End Of Enva Work ##########"
	txt_writing('2ND')
...
if len(sys.argv) == 1 :
	...
	df_final3 = pd.merge(df_final2,df_ac)
	df_final3.to_csv(sam + '_energy_matrix/' + sam + '_total.txt',sep='\t',index=False,na_rep='-')
	os.chdir('../')
