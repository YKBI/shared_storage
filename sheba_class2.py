...
def sheba(a,b):
	...
	os.system('csplit -f \'%s_\' %s.pdb \'/TER/\' --quiet'%(outform2,outform2))
...
def enva_working(pdb):
	...
	os.system('csplit -f \'%s_\' %s.pdb \'/TER/\' --quiet'%(trpdb,trpdb))
...
def red_rev(pdb):
	...
	os.system('csplit -f \'%s_red.pdb_\' %s_red.pdb \"/TER/\" --quiet'%(pppdb,pppdb))
	os.system('python /awork06-1/YKLee/py_script/pep_atom_revi.py ' + PDBLIB + '/' + inpdb + '_native.pdb ' + pppdb + '_red.pdb traj_1')
...
def txt_writing(x):
	...
	acc_columns = ['P%d'%i for i in range(1,seqlen+1)]
        ...
        for f in sorted(glob.glob('*.env')):
                pdb = '_'.join([sam.split('_')[0],str(x),f.split('.')[2].split('_')[0]])
                pdbl.append(pdb)
                with open(f,'r') as F:
                        acc = []
                        psi = []
...
os.system('python /awork06-1/YKLee/py_script/pep_atom_revi.py ' + PDBLIB + '/' + inpdb + '_native.pdb ' + inpdb + '_ref.pdb refer')
