...
def env_working(a,b,c,d,e):
	idx1 = 0
	enva1 = GEAR + '/enva_rec3 -c ' + a + '.pdb > ' + a + '.out'
	enva2 = GEAR + '/enva_rec3 -b ' + a + '.pdb > ' + a + '_bb.out'
	enva3 = GEAR + '/enva_rec3 -a ' + a + '.pdb > ' + a + '_aa.out'
	rmsd = GEAR + '/rmsd_het ligand_feat.txt ' + b + ' ' + a + ' >> feat_rmsd.txt'
	if not os.path.exists(a + '.out'):subprocess.call(enva1,shell=True)
	if not os.path.exists(a + '_bb.out'):subprocess.call(enva2,shell=True)
	if not os.path.exists(a + '_aa.out'):subprocess.call(enva3,shell=True)
	if not os.path.exists('feat_rmsd.txt'):subprocess.call(rmsd,shell=True)
	...
	for i in range(len(d)):
		with open(a + '_aa.out','r') as af:
			for line in af.readlines():
				if line.startswith('ATOM') > 0:
					aenvs = ' '.join(line[26:].split()).split(' ')
					if d[i].split('_')[0] == line[22:26].strip() and d[i].split('_')[1] == line[17:20].strip() and d[i].split('_')[2] == line[12:16].strip():
			...
			if line.startswith('HETATM') > 0:
				...
				if int(envs[8]) == 1:
					...
					if tidx in d:
						...
						num = num + 1
	sidx.sort()
	ratio = float(num) / float(len(d))
	if tot_lig_num > 0:
		ave_lig_acc = tot_lig_acc / float(tot_lig_num)
	else:
		ave_lig_acc = 0

	if not os.path.exists(a + '.ser'):
		with open(a + '.ser', 'w') as ff4:
			for sid in sidx:
				ff4.write('%s\n' % (sid))

	cmd3 = GEAR + '/iskew ' + a + '.ser >> ../' + c + '_energy_matrix/total_sk.txt'
	subprocess.call(cmd3, shell=True)

	with open('../' + c + '_energy_matrix/total_sk1.txt', 'w') as ff5:
		if idx1 == 0:
			ff5.write('PDB\tskewness\tClass\tDecision\n')
		with open('../' + c + '_energy_matrix/total_sk.txt', 'r') as ff6:
			lines = ff6.readlines()
			for line in lines:
				if line.split('\t')[1] == 'nan':
	...
	with open('../' + c + '_energy_matrix/total_rac_ct.txt', 'a') as rf:
		if idx1 == 0:
			rf.write('PDB')
			if len(rfeats_atm) > 0:
		...
		for i in range(len(d)):
			if i == 0:
				rf.write('%s\t%f' % (a, float(raccs[i]) + 1))
			else:
				rf.write('\t%f' % (float(raccs[i]) + 1))
		rf.write('\t%f\t%f\t%f\t%f\n' % (tot_racc, del_racc, ave_lig_acc, ratio))

	n_bb = 0
	with open('../' + c + '_energy_matrix/total_hh_ct.txt', 'a') as hf:
		if idx1 == 0:
			hf.write('PDB\tN.of.BB_full\tN.of.BB_feat\n')
		with open(a + '_bb.out', 'r') as hh:
			hlines = hh.readlines()
			for hline in hlines:
				for i in range(len(d)):
					if d[i].split('_')[0] == hline[22:26].strip() and d[i].split('_')[1] == hline[17:20].strip() and d[i].split('_')[2] == hline[12:16].strip():
						n_bb = n_bb + 1
		hf.write('%s\t%d\t%d\n' % (a, len(hlines), n_bb))
	idx1 = idx1 + 1
...
def acc_chem_count(sam,lfeats,rfeats,ref):
	...
	fff = []
	for i in pdbs:fff.append(i.split('.')[0])
	prod_envs=partial(env_working,c=sam,d=rfeats,b=ref,e=tot_mt_racc)
	pool=multiprocessing.Pool(16)
	atoms = []
	pool.map(prod_envs,fff)
...
if len(sys.argv) == 1 :
	...
	if len(sam.split('_')) > 2 :
		lig = sam.split('_')[0] #sam.split('_')[len(sam.split('_'))-1]
		rec = sam.split('-')[0] #sam.split('_')[0] + '_' + sam.split('_')[1]
	else :
		lig = sam.split('_')[0] #sam.split('_')[1]
		rec = sam.split('-')[0] #sam.split('_')[0]
