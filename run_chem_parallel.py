...
def acc_chem_count(sam,lfeats,rfeats_atm,rfeats_atmn,rfeats_res,rfeats_resn,ref):
	...
	#	subprocess.call(cmdx,shell=True)
		...
		cmd = subprocess.Popen([GEAR,'/enva_rec3','-c',pdb.split('.')[0] + '.pdb','>',pdb.split('.')[0] + '.out'])
		cmd.communicate()
		#subprocess.call(cmd,shell=True)
		#cmd1 = GEAR + '/enva_rec3 -b ' + pdb.split('.')[0] + '.pdb > ' + pdb.split('.')[0] + '_bb.out'
		cmd1 = subprocess.Popen([GEAR,'/enva_rec3','-b',pdb.split('.')[0] + '.pdb','>',pdb.split('.')[0] + '_bb.out'])
		cmd1.communicate()
		#subprocess.call(cmd1,shell=True)
		#cmd3 = GEAR + '/enva_rec3 -a ' + pdb.split('.')[0] + '.pdb > ' + pdb.split('.')[0] + '_aa.out'
		cmd2 = subprocess.Popen([GEAR,'/enva_rec3','-a',pdb.split('.')[0] + '.pdb','>',pdb.split('.')[0] + '_aa.out'])
		cmd2.communicate()
		#subprocess.call(cmd3,shell=True)
		#cmd2 = GEAR + '/rmsd_het ligand_feat.txt ' + ref + ' ' + pdb.split('.')[0] + ' >> feat_rmsd.txt'
		cmd3 = subprocess.Popen([GEAR,'/rmsd_het','ligand_feat.txt',ref,pdb.split('.')[0],'>>','feat_rmsd.txt'])
		cmd3.communicate()
