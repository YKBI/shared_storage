		...
		for i in range(len(rfeats)):
			with open(pdb.split('.')[0] + '_aa.out','r') as af :
				...
				for line in lines:
					...
					#	print '%s\t%s\t%s\t%s\t%s\t%s'%(rfeats[i].split('_')[0],line[22:26].strip(),rfeats[i].split('_')[1],line[17:20].strip(),
		...
		with open('feat_rmsd.txt','r') as ff:
			...
			for line in lines:
				if line.startswith('PDB') > 0 :
					...
					ff1.write('%s\t%s\t%s'%(line.split('\t')[0],line.split('\t')[1],line.split('\t')[2]))
...
def help():
        ...
        print "Usage: python %s [ sample name ] [ receptor index table ]\n"%sys.argv[0]
        return
...
if len(sys.argv) == 1 :
	...
	if len(sam.split('_')) > 2 :
		lig = sam.split('_')[1]#[len(sam.split('_'))-1]
...
#	print rfeats
	...
	df_final3 = pd.merge(df_hh,df_rac)
	...
	df_final3.to_csv(sam + '_energy_matrix/' + lig + '_total.txt',sep='\t',index=False,na_rep='-')
