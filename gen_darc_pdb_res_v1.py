...
with open(rec + '-' + lig + '.pdb','w') as ff2 :
...
cmd1 = GEAR + '/enva_rec3 -c ' + sam + '/' + rec + '.pdb > ' + sam + '/' + rec + '.out'
subprocess.call(cmd1,shell=True)
cmd2 = GEAR + '/enva_rec3 -c ' + rec + '-' + lig + '.pdb > ' + rec + '-' + lig + '.out'
subprocess.call(cmd2,shell=True)

orfeats = []
with open(sam + '/' + rec + '.out','r') as ff4:
	lines = ff4.readlines()
	for line in lines:
		if line.startswith('HETATM') > 0 and line[77]!='H' :
			envs = ' '.join(line[56:].split()).split(' ')
			if int(envs[8]) > 0 :
				orfeats.append(envs[3] + '_' + envs[5] + '_' + envs[6])
num = 0 
ratio = 0
sidx = []
zdx = 0 
with open(rec + '-' + lig + '.out','r') as ff3:
	lines = ff3.readlines()
	for line in lines:
		if line.startswith('HETATM') > 0 :
			zdx = 0
			...
			if tidx in orfeats :
				zdx = orfeats.index(tidx) + 1
				sidx.append(zdx)
				num = num + 1

sidx.sort()
ratio = float(num)/float(len(orfeats))

with open(rec + '-' + lig + '.match','w') as ff6:
	ff6.write('PDB\t%Match\n')
	ff6.write('%s\t%s\n'%(rec + '_' + lig,str(ratio)))

with open(rec + '-' + lig + '.ser','w') as ff4:
	for sid in sidx:
		ff4.write('%s\n'%(sid))

with open(rec + '-' + lig +'_sk.out','w') as ff5:
	ff5.write('PDB\tskewness\tclass\tDecision\n')
cmd3 = GEAR + '/iskew ' + rec + '-' + lig + '.ser >> ' + rec + '-' + lig +'_sk.out' 
subprocess.call(cmd3,shell=True)

df1 = pd.read_csv(rec + '-' + lig + '.match',sep='\t')
df2 = pd.read_csv(rec + '-' + lig + '_sk.out',sep='\t')
df3 = pd.merge(df1,df2)
df3.to_csv(rec + '-' + lig + '_stat.txt',sep='\t',index=False)
os.remove(rec + '-' + lig + '.match')
os.remove(rec + '-' + lig + '.ser')
os.remove(rec + '-' + lig + '.out')
os.remove(rec + '-' + lig +'_sk.out')
