...
if len(sys.argv) == 1 :
	...
	rfeats=[]
	with open(PDBLIB + '/' + rec + '_' + lig + '.out','r') as f1:
		lines = f1.readlines()
		for line in lines:
	...
	acc_chem_count(sam,rfeats,'../crystal_complex')
	...
	total_df1 = [df_hh,df_rac,df_sk]
	df_final1 = reduce(lambda left,right: pd.merge(left,right, on=['PDB'], how='outer'), total_df1)
	df_final1.to_csv(sam + '_energy_matrix/' + sam + '_total.txt',sep='\t',index=False,na_rep='-')
